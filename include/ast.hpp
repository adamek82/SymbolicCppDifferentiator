...
struct Unary : Expr {
    ...
    ExprPtr clone() const override {
        return std::make_unique<Unary>(op, a->clone());
    }
};
...
struct Bin : Expr {
    ...
    ExprPtr clone() const override {
        return std::make_unique<Bin>(op, l->clone(), r->clone());
    }
};
...
struct Pow : Expr {
    ...
    ExprPtr clone() const override {
        return std::make_unique<Pow>(b->clone(), e->clone());
    }
};
...
struct Fun : Expr {
    ...
    ExprPtr clone() const override {
        return std::make_unique<Fun>(f, a->clone());
    }
};
...
inline ExprPtr var(std::string n) {
    return std::make_unique<Var>(std::move(n));
}
inline ExprPtr add(ExprPtr a, ExprPtr b) {
    return std::make_unique<Bin>('+', std::move(a), std::move(b));
}
...
inline ExprPtr divv(ExprPtr a, ExprPtr b) {
    return std::make_unique<Bin>('/', std::move(a), std::move(b));
}
inline ExprPtr neg(ExprPtr a) {
    return std::make_unique<Unary>('-', std::move(a));
}
inline ExprPtr poww(ExprPtr a, ExprPtr b) {
    return std::make_unique<Pow>(std::move(a), std::move(b));
}
inline ExprPtr fun(std::string f, ExprPtr a) {
    return std::make_unique<Fun>(std::move(f), std::move(a));
}

// Safe downcasts used by simplifier
inline const Var* asVar(const Expr* e) {
    return dynamic_cast<const Var*>(e);
}
