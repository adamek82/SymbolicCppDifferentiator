...
static ExprPtr simplifyBin(char op, ExprPtr L, ExprPtr R) {
    ...
    if (op == '+') {
        if (isNumber(L.get(), &a) && isNumber(R.get(), &b))
            return num(a + b);
        if (isNumber(L.get(), &a) && a == 0)
            return R;
        if (isNumber(R.get(), &b) && b == 0)
            return L;
    ...
    if (op == '-') {
        ...
        if (isNumber(R.get(), &b) && b == 0)
            return L;
        if (isNumber(L.get(), &a) && a == 0)
            return std::make_unique<Unary>('-', std::move(R));
        return std::make_unique<Bin>(op, std::move(L), std::move(R));
    }
    if (op == '*') {
        if (isNumber(L.get(), &a) && isNumber(R.get(), &b))
        ...
        if (isNumber(R.get(), &b) && b == 0)
            return num(0);
        if (isNumber(L.get(), &a) && a == 1)
            return R;
        if (isNumber(R.get(), &b) && b == 1)
            return L;
        if (isNumber(L.get(), &a) && a == -1)
            return std::make_unique<Unary>('-', std::move(R));
        if (isNumber(R.get(), &b) && b == -1)
            return std::make_unique<Unary>('-', std::move(L));
    ...
    if (op == '/') {
        ...
        if (auto lb = dynamic_cast<Bin*>(L.get()); lb && lb->op == '*') {
            ...
            if (asVar(lb->r.get())) {
                if (auto pr = dynamic_cast<Pow*>(R.get())) {
                    if (asVar(pr->b.get()) &&
                        asVar(lb->r.get())->name == asVar(pr->b.get())->name) {
                        if (isNumber(pr->e.get(), &b))
                            return simplifyRec(mul(
                                lb->l->clone(), poww(var(asVar(lb->r.get())->name), num(1 - b))));
                    }
                }
            }
            // (y*A) / y^n  -> A * y^(1-n)
            if (asVar(lb->l.get())) {
                if (auto pr = dynamic_cast<Pow*>(R.get())) {
                    if (asVar(pr->b.get()) &&
                        asVar(lb->l.get())->name == asVar(pr->b.get())->name) {
                        if (isNumber(pr->e.get(), &b))
                            return simplifyRec(mul(
                                lb->r->clone(), poww(var(asVar(lb->l.get())->name), num(1 - b))));
                    }
                }
            }
        }
...
static ExprPtr simplifyPow(ExprPtr B, ExprPtr E) {
    B = simplifyRec(std::move(B));
    E = simplifyRec(std::move(E));
    double a, b;
    if (isNumber(E.get(), &b)) {
        if (b == 0)
            return num(1);
...
static ExprPtr simplifyFun(std::string f, ExprPtr A) {
    ...
    if (auto n = dynamic_cast<Num*>(A.get())) {
        double x = n->v;
        if (f == "sin")
            return num(std::sin(x));
        if (f == "cos")
            return num(std::cos(x));
        if (f == "tan")
            return num(std::tan(x));
        if (f == "ln")
            return num(std::log(x));
        if (f == "log")
            return num(std::log10(x));
        if (f == "exp")
            return num(std::exp(x));
    }
    return std::make_unique<Fun>(std::move(f), std::move(A));
}

static ExprPtr simplifyRec(ExprPtr e) {
    if (dynamic_cast<Num*>(e.get()) || dynamic_cast<Var*>(e.get()))
        return e;
