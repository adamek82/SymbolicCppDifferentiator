...
static bool isConst(const Expr* e, const std::string& dx){
    ...
    if(auto v=dynamic_cast<const Var*>(e)) return v->name!=dx;
    if(auto u=dynamic_cast<const Unary*>(e)) return isConst(u->a.get(), dx);
    if(auto b=dynamic_cast<const Bin*>(e))  return isConst(b->l.get(),dx) && isConst(b->r.get(),dx);
    if(auto p=dynamic_cast<const Pow*>(e))  return isConst(p->b.get(),dx) && isConst(p->e.get(),dx);
    if(auto f=dynamic_cast<const Fun*>(e))  return isConst(f->a.get(),dx);
    return false;
}
...
ExprPtr differentiate(const Expr* e, const std::string& dx){
    ...
    if(auto u=dynamic_cast<const Unary*>(e)){
        if(u->op=='-') return neg(differentiate(u->a.get(), dx));
    }

    if(auto b=dynamic_cast<const Bin*>(e)){
        if(b->op=='+') return add(differentiate(b->l.get(),dx), differentiate(b->r.get(),dx));
        if(b->op=='-') return sub(differentiate(b->l.get(),dx), differentiate(b->r.get(),dx));
        if(b->op=='*')
    ...
    if(auto f=dynamic_cast<const Fun*>(e)){
        ...
        if(fn=="sin") return mul(fun("cos", f->a->clone()), std::move(du));
        if(fn=="cos") return mul(neg(fun("sin", f->a->clone())), std::move(du));
        if(fn=="tan") return mul(divv(num(1), poww(fun("cos", f->a->clone()), num(2))), std::move(du));
        if(fn=="ln")  return mul(divv(num(1), f->a->clone()), std::move(du));
        if(fn=="log") return mul(divv(num(1), mul(f->a->clone(), fun("ln", num(10)))), std::move(du));
        if(fn=="exp") return mul(fun("exp", f->a->clone()), std::move(du));
